...
1. Пользователь пишет `8500` или `15.06 10000`.
2. Бот парсит дату (или берёт вчера) и шаги.
3. Без фото — сразу записывает/обновляет запись.

### Отправка скриншота + шаги
1. Пользователь отправляет фото (с подписью-числом или отдельно число после фото).
2. Бот скачивает фото локально.
3. Бот показывает inline-клавиатуру с датами за последние 7 дней:
   - `15.06 ✅ (Перезаписать?)` — если запись за дату уже есть
   - `15.06 (Не заполнено)` — если записи нет
4. После выбора даты бот загружает скриншот в S3 с именем `Пользователь-ДД.ММ-шаги.jpg` и записывает/обновляет запись.
