...
2. Бот показывает inline-клавиатуру с датами за последние 14 дней (`DATE_SELECTION_DAYS` в `bot.py`) и кнопку «✍️ Указать дату вручную».
3. Пользователь выбирает дату кнопкой или нажимает «✍️» и вводит дату текстом (`ДД.ММ` или `ДД.ММ.ГГГГ`, проверяется через `normalize_date` и рамки конкурса).
