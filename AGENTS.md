...
2. Бот показывает inline-клавиатуру с датами за последние 14 дней (`DATE_SELECTION_DAYS` в `bot.py`) и подсказку, что дату можно ввести вручную (например, `03.07`).
3. Пользователь выбирает дату кнопкой, нажимает «✍️ Указать дату вручную» или просто вводит дату текстом (`ДД.ММ` или `ДД.ММ.ГГГГ`) — пока активна сессия выбора даты, текст-дата распознаётся автоматически (проверка через `normalize_date` и рамки конкурса).
4. После выбора/ввода даты бот записывает/обновляет запись.
...
6. **Ограничение выбора даты кнопками — только последние 14 дней**
   - Inline-клавиатура показывает только последние две недели (`DATE_SELECTION_DAYS` в `bot.py`).
   - Для более ранних дат пользователь может нажать «✍️ Указать дату вручную» и ввести дату текстом — ограничение только рамки конкурса (`CONTEST_START_DATE` / `CONTEST_END_DATE`).
